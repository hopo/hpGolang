...
func main() {
	...
	// launches goroutines
	// return channels which will eventually be closed
	...
	// NOTE EIGHT
	// our program in done
	// flows of code exits out of main
	// program ends
}
...
func incrementor(s string) <-chan string {
	...
	go func() {
		...
		// pulling values off the cahnnel
		// ***************************
		// close the channel
		// return that closed channel
		// these "incrementor" goroutines are off running
		close(c)
	}()
	return c
}
